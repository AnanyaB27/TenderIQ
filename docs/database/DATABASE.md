...
TenderIQ uses PostgreSQL with the `pgvector` extension. Migrations are strictly managed via TypeORM.

## Core Entities

- `organization` / `organization_members` / `users`: P1.3 Identity & Tenant Isolation.
- `msme_profiles` / `msme_certifications`: P1.7 Factual ground truth for matching.
- `tenders` / `tender_sources`: P1.6 Normalized ingestion tables.
- `tender_documents`: Tracks PDF status (`PROCESSING`, `READY`, `NO_EXTRACTABLE_TEXT`).
- `tender_document_chunks`: Contains `vector(768)` embedding column, page metadata, and hash.
- `tender_evaluations`: Persists match scores, evidence coverage, rule results, `confidenceMetadata` (JSONB), and `riskFlags` (JSONB).
- `tender_drafts`: Persists AI-assisted bid drafts safely linked to the tenant organization.
