...
```json
{
  "data": {
    ...
    "valueWonAmount": 12500000.0,
    "valueBidAmount": 38900000.0,
    "currency": "INR"
  }
}
```
...
All routes below require JWT Bearer Authentication and are isolated by `OrganizationMembershipGuard`.

## Tenders & Discovery

- `GET /tenders?search={text}&page={n}` - Database-backed pagination and search.
- `GET /organizations/:orgId/tenders/sync-live` - Triggers CPPP ingestion.

## Documents

- `POST /organizations/:orgId/documents/extract` - Uploads PDF, triggers processing, returns summary.

## Evaluation & Risk

- `POST /organizations/:orgId/tenders/:tenderId/evaluate` - Runs end-to-end matching pipeline.
- `GET /organizations/:orgId/tenders/:tenderId/evaluation` - Retrieves persisted score, evidence, and Risk/Confidence metadata.

## Drafting

- `POST /organizations/:orgId/tenders/:tenderId/drafts` - Generates grounded bid sections.
